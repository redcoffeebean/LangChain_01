...
def answer_without_rag(question: str, openai_api_key: str) -> str: 
    ...
    sys = SystemMessage(content="너는 간결한 조수다. 모든 답변은 2~3문장 이내로 핵심만 요약해서 말해라. 현재 너의 기본 LLM 상태에서 모르는 것에 대해서는 별도로 웹 검색은 금지한다. 그리고 현재로써 모르고 알 수 없다면 모른다고 알 수 없다고 사실대로 말해라.")    # 프롬프트 중 시스템 롤에 해당(role="system")
