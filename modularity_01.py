...
# --- Active Vector Index banner (minpatch) ---
...
if active_vs and getattr(active_vs, "vs", None) is not None:
    ...
    if index is not None:
        ...
        st.success(
            f"Active Vector Index — {type(vsv).__name__.upper()} | "
            f"vectors={ntotal} | dim={dim if dim is not None else '-'} | index={idx_type}"
        )
else:
    st.info("Active Vector Index 없음. 먼저 인덱스를 빌드하세요.")
uploaded_files = st.file_uploader(key="uploader", "문서 업로드 (PDF/DOCX/PPT/TXT)", type=["pdf", "docx", "doc", "pptx", "ppt", "txt"], accept_multiple_files=True)
